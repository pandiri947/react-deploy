...
class Details extends React.Component {
    ...
    componentDidMount() {
        ...
        axios({
            url: `http://localhost:2020/api/getResById/${resId}`,
            method: 'GET',
    ...
    handleClick = (state, value) => {
        ...
        if (state === 'orderModalIsOpen') {
            axios({
                url: `http://localhost:2020/api/getItemsbyrestaurant/${restaurantId}`,
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            }).then(res => {
    ...
    makePayment = (e) => {
        ...
        this.getData({ amount: subTotal, email: email }).then(response => {
            var information = {
                action: "https://localhost:2020/ securegw-stage.paytm.in/order/process",
                params: response
            }
